    ...
    <>
      ...
      <form action={formAction} className="mt-6 grid gap-4 md:grid-cols-2">
        <label className="text-sm font-medium text-slate-700">Class name
          <input name="name" required className={`mt-2 w-full rounded-2xl border px-4 py-3 ${state?.errorFields?.includes("name") ? "border-rose-400" : "border-slate-200"}`} placeholder="Room 22 Fraction Lab" />
        </label>
        ...
        <label className="text-sm font-medium text-slate-700">Student count
          <input name="studentCount" type="number" min={1} required className={`mt-2 w-full rounded-2xl border px-4 py-3 ${state?.errorFields?.includes("studentCount") ? "border-rose-400" : "border-slate-200"}`} placeholder="24" />
        </label>
